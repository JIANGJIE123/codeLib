...
(function($){
	...
	window.confirm = function(msg,okCall,cancelCall){
		window.parent.wxc.xcConfirm(msg,"confirm",{
			onOk:okCall,
			onCancel:cancelCall
		});
	}
	...
	window.loading = function(msg){
		$(window.parent.document.body).find("#loadMask").remove();
		var _htm = '<div id="loadMask" style="width: 100%;height: 100%;background-color:#fff;position: fixed;top: 0px;left: 0px;opacity: 0.1;"></div>';
		$(window.parent.document.body).append(_htm);
	}
